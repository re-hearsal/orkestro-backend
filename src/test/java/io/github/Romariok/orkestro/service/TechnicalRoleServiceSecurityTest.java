...
class TechnicalRoleServiceSecurityTest {
      ...
      @Test
      void createOrganizationRole_hasExpectedPreAuthorizeExpression() throws NoSuchMethodException {
            Method method = TechnicalRoleService.class
                        .getMethod("createOrganizationRole", Long.class, TechnicalRoleCreateRequestDTO.class);

            PreAuthorize preAuthorize = method.getAnnotation(PreAuthorize.class);

            String expr = preAuthorize.value();
            assertEquals(
                        "hasAuthority('CTX_PERM_ORG:' + #organizationId + ':ORG_TECH_ROLE_MANAGE')",
                        expr);
      }

      @Test
      void createSectionRole_hasExpectedPreAuthorizeExpression() throws NoSuchMethodException {
            Method method = TechnicalRoleService.class
                        .getMethod("createSectionRole", Long.class, TechnicalRoleCreateRequestDTO.class);

            PreAuthorize preAuthorize = method.getAnnotation(PreAuthorize.class);

            String expr = preAuthorize.value();
            assertEquals(
                        "hasAuthority('CTX_PERM_SECTION:' + #sectionId + ':SECTION_TECH_ROLE_MANAGE')",
                        expr);
      }
